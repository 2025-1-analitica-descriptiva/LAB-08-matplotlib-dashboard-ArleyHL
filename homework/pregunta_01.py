...
def pregunta_01():
    ...
    * `Weight_in_gms`
    ...
    https://github.com/jdvelasq/LAB_matplotlib_dashboard/blob/main/shiv` contiene información sobre los envios
    de productos de una empresa. Cree un dashboard estático en HTML que
    permita visualizar los siguientes campos:
    ...
    * `Weight_in_gms`
    ...
    https://github.com/jdvelasq/LAB_mav` contiene información sobre los envios
    de productos de una empresa. Cree un dashboard estático en HTML que
    permita visualizar los siguientes campos:
    ...
    * `Weight_in_gms`
    ...
    https://github.com/jdvelasq/LAB_mapping-dashboard-example.png
...
def create_visual_for_shipping_per_warehouse(df):
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "shipping_per_warehouse.png"), dpi=150)
    plt.close()
...
def create_visual_for_mode_of_shipment(df):
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "mode_of_shipment.png"), dpi=150)
    plt.close()
...
def create_visual_for_average_customer_rating(df):
    ...
    plt.figure()
    summary = (df[["Mode_of_Shipment", "Customer_rating"]] .groupby("Mode_of_Shipment") .describe())
    summary.columns = summary.columns.droplevel()
    summary = summary[["mean", "min", "max"]]

    plt.barh(y=summary.index, width=summary["max"] - 1, left=summary["min"], height=0.9, color="lightgray", alpha=0.8, edgecolor="black") 
    colors = ["tab:green" if val >= 3.0 else "tab:orange" for val in summary["mean"]]
    plt.barh(y=summary.index, width=summary["mean"] - 1, left=summary["min"], color=colors, height=0.5, alpha=1.0, edgecolor="black")
